...
m_in = 0 # Mass flow rate of hydrogen / kg / s
...
charge_temperature = CSV.read("outputs/charge_temperature_profiles.csv", DataFrame)
charge_final_temp = charge_temperature[!, Symbol("t_1042.0s")]
charge_timeseries_data = CSV.read("outputs/charge_timeseries_data.csv", DataFrame)
...
t₀ = 1042 # Initial time // s
t_f = 3189 # Final time // s
...
df_exp_middle_temp = CSV.read("inputs/xiao_middle_temp_finite_element_dormant.csv", DataFrame)
df_exp_center_temp = CSV.read("inputs/xiao_center_temp_finite_element_dormant.csv", DataFrame)
df_exp_wall_temp = CSV.read("inputs/xiao_wall_temp_finite_element_dormant.csv", DataFrame)

# Pressure profile
df_exp_pressure = CSV.read("inputs/xiao_pressure_finite_element_dormant.csv", DataFrame)

# Mass profiles
df_exp_gas_mass = CSV.read("inputs/xiao_gas_mass_finite_element_dormant.csv", DataFrame)
df_exp_adsorbed_mass = CSV.read("inputs/xiao_adsorbed_mass_finite_element_dormant.csv", DataFrame)
df_exp_total_mass = CSV.read("inputs/xiao_total_mass_finite_element_dormant.csv", DataFrame)
...
output_filename = "outputs/dormant_simulation_profiles.svg"
savefig(final_plot, output_filename)
println("\nPlot saved successfully as '$output_filename'")

# Save individuals
savefig(p_temp, "outputs/dormant_julia_temperature_profile.svg")
savefig(p_pressure, "outputs/dormant_julia_pressure_profile.svg")
savefig(p_mass, "outputs/dormant_julia_mass_profiles.svg")
...
CSV.write("outputs/dormant_temperature_profiles.csv", df_temp)
...
CSV.write("outputs/dormant_adsorption_profiles.csv", df_adsorption)
...
CSV.write("outputs/dormant_timeseries_data.csv", df_timeseries)
